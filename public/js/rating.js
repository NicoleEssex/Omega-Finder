...
$(document).ready(function() {
  ...
  // A function for handling what happens when the form to create a new rating is submitted
  function handleFormSubmit(event) {
    event.preventDefault();
    
    // Wont submit the post if we are missing stars
    if (!starsInput.val().trim()) {
      return;
    }
    // Constructing a newRatings object to hand to the database
    var newRatings = {
      stars: starsInput.val().trim(),
  ...
  function submitRatings(Ratings) {
    $.post("/api/rating/create/", Ratings, function() {
      window.location.href = "/";
    });
  }
  
// This is all Shawns code
// // Initial Ratings
// const ratings = {
//     Sev: 4.7,
//     walmart: 3.4,
